function []=defineTopology_simple_1path(N,k,G)
...
load(sprintf('topo_params_N=%d_k=%d_G=%d.mat',N,k,G));
...
filename=sprintf('dynexp_1path_N=%d_k=%d_G=%d.txt',N,k,G);
fID=fopen(filename,'w');

fprintf(fID,'%d %d %d %d\n',N*d,d,u,N); % hosts, downlinks, uplinks, racks
fprintf(fID,'%d %d %d %d',slicetypes*N/G,ceil(eps0),ceil(del),ceil(r)); % # slices. (eps, delta, r) * Ntors

%%

for superslice=1:N/G
    
    fprintf(sprintf('Writephase 1, slice %d of %d\n',superslice,N/G));
...
% with only 1 path per pair:
for sliceind=1:N/G
    
    fprintf(sprintf('Writephase 2, slice %d of %d\n',sliceind,N/G));
        ...
        for srctor=1:N
            for dsttor=1:N
                if srctor~=dsttor
                    ...
                    [~,nhop]=size(paths{sliceind}{srctor,dsttor});
                    for hop=1:nhop-1
                        s_src=paths{sliceind}{srctor,dsttor}(hop); % switch source port (rack)
                        s_dst=paths{sliceind}{srctor,dsttor}(hop+1); % switch output port (rack)
                        foundswitch=0;
                        qind=0;
                        ...
                        if hop~=nhop-1
                            fprintf(fID,'%d ',u + qind-1); % indexed from zero
                        else
                            fprintf(fID,'%d',u + qind-1);
                        end
                    end
                end
            end
