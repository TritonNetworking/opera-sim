...
% Update: Max Mellette 11/30/2017 - calls dijkstra_vector() way faster!
...
%     [path cost] = dijkstra(netCostMatrix, source, destination);
    A=netCostMatrix;
    A(A==inf)=0;
    [cost,path]=dijkstra_vector(A,A,source,destination);
...
%                 [dev_p c] = dijkstra(temp_netCostMatrix, P_(index_dev_vertex), destination);
                A=temp_netCostMatrix;
                A(A==inf)=0;
                [c,dev_p]=dijkstra_vector(A,A,P_(index_dev_vertex),destination);
