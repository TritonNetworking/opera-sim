...
#include "fat_tree_topology_3to1_k24.h"
#include <vector>
#include "string.h"
#include <sstream>
#include <strstream>
...
void FatTreeTopology::set_params(int no_of_nodes) {
    ...
    cout << "Building 3:1 Fat Tree with 5,184 nodes (k=24)" << endl;

    Nlp = 288; // number of ToRs
    Nup = 144; // number of upper pod switches
    Nc = 72; // number of core switches
    Nh = _no_of_nodes; // number of hosts
    Nhpr = 18; // number of hosts per rack
...
void FatTreeTopology::init_network(){
  ...
  for (int k = 0; k < Nup; k++) { // sweep upper pod switch id
    for (int l = 0; l < 12; l++) { // sweep lower pod (12 ToRs per pod)

      int pod = k / 6; // get the current pod (6 upper pod switches per pod)
      int j = pod * 12 + l; // !!! (there are 12 ToRs per pod)
  ...
  for (int j = 0; j < Nup; j++) { // sweep upper pod switch ID
    ...
    int pod = j / 6; // get current pod (there are 6 upper pod switches per pod)
    int switch_pos = j % 6; // get switch position within pod

    for (int l = 0; l < 12; l++) { // there are 12 uplinks

      int k = switch_pos * 12 + l; // get the core switch ID
...
vector<const Route*>* FatTreeTopology::get_paths(int src, int dest){
  ...
  if (src / Nhpr == dest / Nhpr) { // within the same rack
  ...
  else if (src / 216 == dest / 216) { // within the same pod (there are 216 hosts within a pod)
    //don't go up the hierarchy, stay in the pod only.

    int pod = src / 216; // get the pod

    //there are 6 paths between the source and the destination
    for (int u = 0; u < 6; u++){
      // upper is nup

      int upper = pod * 6 + u; // get the upper switch ID
    ...
    // there are 6 ToR uplinks
    for (int u = 0; u < 6; u++) {

      int upper = pod * 6 + u; // get the upper switch ID

      // there are 12 upper pod uplinks
      for (int c = 0; c < 12; c++){

        int core = u * 12 + c; // get the core switch ID
        ...
        int pod2 = dest / 216; // get the pod
      	int upper2 = pod2 * 6 + u; // there are 6 uplinks per ToR
