...
void FatTreeTopology::set_params(int no_of_nodes) {
    ...
    cout << "Building 3:1 Fat Tree with 648 nodes (k=12)" << endl;

    Nlp = 72; // number of ToRs
    Nup = 36; // number of upper pod switches
    Nc = 18; // number of core switches
    Nh = _no_of_nodes; // number of hosts
    Nhpr = 9; // number of hosts per rack
...
void FatTreeTopology::init_network(){
  ...
  for (int k = 0; k < Nup; k++) { // sweep upper pod switch id
    for (int l = 0; l < 6; l++) { // sweep lower pod

      int pod = k / 3; // get the current pod
  ...
  for (int j = 0; j < Nup; j++) { // sweep upper pod switch ID
    ...
    int pod = j / 3; // get current pod
    int switch_pos = j % 3; // get switch position within pod
...
vector<const Route*>* FatTreeTopology::get_paths(int src, int dest){
  ...
  else if (src / 54 == dest / 54) { // within the same pod
    //don't go up the hierarchy, stay in the pod only.

    int pod = src / 54; // get the pod

    //there are 3 paths between the source and the destination
    for (int u = 0; u < 3; u++){
      // upper is nup

      int upper = pod * 3 + u; // get the upper switch ID
