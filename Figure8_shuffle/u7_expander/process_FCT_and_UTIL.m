...
fname=sprintf('output_100kB_pull1_cwnd40_q46.txt');
...
while ischar(string)
    if strcmp(string(1:3),'FCT')
        cnt_line=cnt_line+1;
        cnt_fct=cnt_fct+1;
        data_fct_u7exp(cnt_fct,:)=sscanf(string,'%*s %d %d %d %f %f %*\n').';
        % src, dst, bytes, fct, time_started
        string=fgetl(fid);
    elseif strcmp(string(1:4),'Util')
        cnt_line=cnt_line+1;
        cnt_util=cnt_util+1;
        data_util_u7exp(cnt_util,:)=sscanf(string,'%*s %f %f %*\n').';
        string=fgetl(fid);
    else
        cnt_line=cnt_line+1;
...
Nrecorded_flows=length(data_fct_u7exp(:,1));

% ---

figure;
hold on;
plot(data_util_u7exp(:,2),data_util_u7exp(:,1),'-o','linewidth',2);
ylim([0 1]);
ax=gca;
ax.FontSize=16;
xlabel('Time (ms)');
...
save('u7exp_100kB_only.mat','data_fct_u7exp','data_util_u7exp');
