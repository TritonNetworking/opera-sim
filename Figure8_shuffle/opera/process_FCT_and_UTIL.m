...
% fname=sprintf('sim/output_100kB_opera_1path_1hopOnly.txt');
% matname='Opera_100kB_only_1path_1hop.mat';

fname=sprintf('sim/output_100kB_opera_5paths_1hopOnly.txt');
matname='Opera_100kB_only_5paths_1hop.mat';
...
while ischar(string)
    if strcmp(string(1:3),'FCT')
        cnt_line=cnt_line+1;
        cnt_fct=cnt_fct+1;
        data_fct_opera(cnt_fct,:)=sscanf(string,'%*s %d %d %d %f %f %*\n').';
        % src, dst, bytes, fct, time_started
        string=fgetl(fid);
    elseif strcmp(string(1:4),'Util')
        cnt_line=cnt_line+1;
        cnt_util=cnt_util+1;
        data_util_opera(cnt_util,:)=sscanf(string,'%*s %f %f %*\n').';
        string=fgetl(fid);
    else
        cnt_line=cnt_line+1;
...
Nrecorded_flows=length(data_fct_opera(:,1));

% ---

figure;
hold on;
plot([data_util_opera(:,2);(101:1:300).'], ...
    [data_util_opera(:,1);zeros(1,200).'],'-o','linewidth',2);
ylim([0 1]);
ax=gca;
ax.FontSize=16;
xlabel('Time (ms)');
...
grid on;
box on;
% xlim([data_util_opera(1,2) data_util_opera(end,2)]);
xlim([0 300]);
...
% save(matname,'data_fct_opera','data_util_opera');
