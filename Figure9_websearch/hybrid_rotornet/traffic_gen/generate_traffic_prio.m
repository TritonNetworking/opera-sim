...
% !!! NOTE: This generates only the flows that would go over Opera paths
% to emulate priority queuing
...
loadfrac0=.08; % fraction of theoretically possible load

% ----- define simulation length (time):

totaltime=10.001; % seconds, probably want a little extra here...
v=1;
filename=sprintf('flows_%.0fpercLoad_10sec_%dhosts_6to1_hybrid_prio_%d',100*loadfrac0,H,v);
...
%     [~,ind]=min(abs(flowcdf-randvect(a))); % this was skewing the effective rate too low...
    ...
    if flowsize(ind) < 100e6 % set all flows to be prio for websearch
        flowmat1(a-offset,3)=flowsize(ind);
    else
        flowmat1(a-offset,:)=[]; % delete this flow
        offset = offset + 1;
    end
end
% we deleted some, so update the number of flows:
[nflows,~]=size(flowmat1);
